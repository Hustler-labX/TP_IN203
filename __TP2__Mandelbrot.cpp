...
std::vector<int> computeMandelbrotSet( int W, int H, int maxIter)
{
   ...
   int rank, nbp;
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &nbp);
	    ...
	    for ( int i = rank*H/nbp; i < (rank+1)*H/nbp; ++i ) computeMandelbrotSetRow(W, H, maxIter, i, pixels.data() + W*(i-rank*H/nbp));
 ...
 { 
	...
	if (rank == 0)
	{
	  	auto iters = computeMandelbrotSet(W, H,maxIter );
        	double scaleCol = 1./maxIter;//16777216
        	std::ofstream ofs( "mandelbroddt.tga", std::ios::out | std::ios::binary );
        	ofs << "P6\n"<< W << " " << H << "\n255\n";
		for ( int i = 0; i < W * H/nbp; ++i ) 
		{
        	...
        	{
		    MPI_Recv (&iters[0] , W * (H*(compt+1)/nbp  - H*compt/nbp), MPI_INT , compt, 0, MPI_COMM_WORLD ,& status );
		    for ( int i = 0; i < W * (H*(compt+1)/nbp  - H*compt/nbp); ++i ) 
		    	{
				double iter = scaleCol*iters[i];
				unsigned char r = (unsigned char)(256 - (unsigned (iter*256.) & 0xFF));
				unsigned char b = (unsigned char)(256 - (unsigned (iter*65536) & 0xFF));
        ...
        std::cout << "Temps calcul ensemble mandelbrot pour processeur: " << rank << elapsed_seconds.count() << std::endl;
    }
    
    else
    {
        std::cout <<"Getting into  processeur n" << rank << std::endl;
        auto iters = computeMandelbrotSet(W, H,maxIter );
        MPI_Send(&iters[0], W * (H*(rank+1)/nbp  - H*rank/nbp), MPI_INT, 0, 0, MPI_COMM_WORLD);
	auto end = std::chrono::system_clock::now();
        std::chrono::duration<double> elapsed_seconds = end-start;
        std::cout << "Temps calcul ensemble mandelbrot pour processeur( " << rank << " ) "<<elapsed_seconds.count() << std::endl;
    }
